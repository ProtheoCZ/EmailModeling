...
TIME_EVOLUTION_RESULT_FOLDER_PATH = '../emailModelingBe/rumourTimeEvolution/TimeEvolution/'
...
# function to plot heat map for rumour spread model
def plot_heat_map(param_name, result_folder_path, graph_name):
    delta_values = [round(delta, 2) for delta in list(pd.np.arange(0.02, 0.22, 0.02))]  # delta range
    alfa_values = [round(alfa, 2) for alfa in list(pd.np.arange(0.02, 0.22, 0.02))]  # alfa range
    ...
    ax = sns.heatmap(df, vmin=0, vmax=500) #set limits for each graph differently
    ax.invert_yaxis()
...
# function to plot time evolution for rumour spread model
...
plot_heat_map(AVG_NODE_COUNT, RESULT_FOLDER_PATH_DEFAULT, EU_CORE)
